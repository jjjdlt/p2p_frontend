...
async function loadTamagui(propsIn) {
  ...
  if (await generateThemesAndLog(props, !0), !hasBundledConfigChanged())
    return bundleInfo;
  if (bundleInfo) {
    const { createTamagui } = requireTamaguiCore(props.platform || "web"), config = createTamagui(bundleInfo.tamaguiConfig), { outputCSS } = props;
...
`)) : SHOULD_DEBUG && console.error(err) : console.error("Error loading tamagui.config.ts", err);
      const { createTamagui } = requireTamaguiCore(props.platform || "web"), { getDefaultTamaguiConfig } = require("@tamagui/config-default");
      return {
        components: [],
...
function resolveWebOrNativeSpecificEntry(entry) {
  const workspaceRoot = resolve(), resolved = require.resolve(entry, { paths: [workspaceRoot] }), ext = extname(resolved), fileName = basename(resolved).replace(ext, ""), specificFile = join(dirname(resolved), fileName + "." + "web" + ext);
