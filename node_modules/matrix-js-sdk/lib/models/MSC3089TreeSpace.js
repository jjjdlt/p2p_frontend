...
import { averageBetweenStrings, DEFAULT_ALPHABET, lexicographicCompare, nextString, prevString, simpleRetryOperation } from "../utils";
...
export class MSC3089TreeSpace {
  ...
  invite(userId) {
    ...
    return _asyncToGenerator(function* () {
      ...
      var shareHistoryKeys = _arguments.length > 2 && _arguments[2] !== undefined ? _arguments[2] : true;
      var promises = [_this2.retryInvite(userId)];
      if (andSubspaces) {
        promises.push(..._this2.getDirectories().map(d => d.invite(userId, andSubspaces, shareHistoryKeys)));
      }
      return Promise.all(promises).then(() => {
        ...
        if (shareHistoryKeys && isRoomSharedHistory(_this2.room)) {
          // noinspection JSIgnoredPromiseFromCall - we aren't concerned as much if this fails.
          _this2.client.sendSharedHistoryKeys(_this2.roomId, [userId]);
        }
      });
    })();
  }
