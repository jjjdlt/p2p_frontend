...
import { VerificationPhase, VerificationRequestEvent, VerifierEvent } from "../crypto-api/verification";
...
export class RustVerificationRequest extends TypedEventEmitter {
  ...
  constructor(olmMachine, inner, outgoingRequestProcessor, supportedVerificationMethods) {
    ...
    var onChange = /*#__PURE__*/function () {
      var _ref = _asyncToGenerator(function* () {
        var verification = _this.inner.getVerification();
        ...
        if (verification instanceof RustSdkCryptoJs.Sas) {
          if (_this._verifier === undefined || _this._verifier instanceof RustQrCodeVerifier) {
            _this.setVerifier(new RustSASVerifier(verification, _this, outgoingRequestProcessor));
          } else if (_this._verifier instanceof RustSASVerifier) {
            _this._verifier.replaceInner(verification);
          }
        } else if (verification instanceof RustSdkCryptoJs.Qr && _this._verifier === undefined) {
          _this.setVerifier(new RustQrCodeVerifier(verification, outgoingRequestProcessor));
        }
        _this.emit(VerificationRequestEvent.Change);
      });
  ...
  getOtherDevice() {
    ...
    return _asyncToGenerator(function* () {
      var otherDeviceId = _this2.inner.otherDeviceId;
      if (!otherDeviceId) {
        return undefined;
      }
      return yield _this2.olmMachine.getDevice(_this2.inner.otherUserId, otherDeviceId, 5);
    })();
  }
  ...
  accept() {
    var _this3 = this;
    return _asyncToGenerator(function* () {
      if (_this3.inner.phase() !== RustSdkCryptoJs.VerificationRequestPhase.Requested || _this3._accepting) {
        throw new Error("Cannot accept a verification request in phase ".concat(_this3.phase));
      }
      _this3._accepting = true;
      try {
        var req = _this3.inner.acceptWithMethods(_this3.supportedVerificationMethods.map(verificationMethodIdentifierToMethod));
        if (req) {
          yield _this3.outgoingRequestProcessor.makeOutgoingRequest(req);
        }
      } finally {
        _this3._accepting = false;
      }

      // phase may have changed, so emit a 'change' event
      _this3.emit(VerificationRequestEvent.Change);
    })();
  }
  ...
  cancel(params) {
    var _this4 = this;
    return _asyncToGenerator(function* () {
      if (_this4._cancelling) {
        // already cancelling; do nothing
        return;
      }
      _this4._cancelling = true;
      try {
        var req = _this4.inner.cancel();
        if (req) {
          yield _this4.outgoingRequestProcessor.makeOutgoingRequest(req);
        }
      } finally {
        _this4._cancelling = false;
      }
    })();
  }
  ...
  startVerification(method) {
    ...
    return _asyncToGenerator(function* () {
      ...
      if (!(yield _this5.getOtherDevice())) {
        throw new Error("startVerification(): other device is unknown");
      }
      var res = yield _this5.inner.startSas();
      if (res) {
        var [, req] = res;
  ...
  scanQRCode(uint8Array) {
    ...
    return _asyncToGenerator(function* () {
      ...
      var verifier = yield _this6.inner.scanQrCode(scan);
      ...
      if (req) {
        yield _this6.outgoingRequestProcessor.makeOutgoingRequest(req);
      }
      return _this6._verifier;
    })();
  }
  ...
  generateQRCode() {
    ...
    return _asyncToGenerator(function* () {
      // make sure that we have a list of the other user's devices (workaround https://github.com/matrix-org/matrix-rust-sdk/issues/2896)
      if (!(yield _this7.getOtherDevice())) {
        throw new Error("generateQRCode(): other device is unknown");
      }
      var innerVerifier = yield _this7.inner.generateQrCode();
...
export class RustQrCodeVerifier extends BaseRustVerifer {
  ...
  verify() {
    ...
    return _asyncToGenerator(function* () {
      ...
      if (_this9.callbacks !== null) {
        _this9.emit(VerifierEvent.ShowReciprocateQr, _this9.callbacks);
      }
  ...
  confirmScanning() {
    var _this10 = this;
    return _asyncToGenerator(function* () {
      var req = _this10.inner.confirmScanning();
      if (req) {
...
export class RustSASVerifier extends BaseRustVerifer {
  ...
  verify() {
    var _this11 = this;
    return _asyncToGenerator(function* () {
      yield _this11.sendAccept();
      yield _this11.completionDeferred.promise;
    })();
  }
  ...
  sendAccept() {
    var _this12 = this;
    return _asyncToGenerator(function* () {
      var req = _this12.inner.accept();
      if (req) {
        yield _this12.outgoingRequestProcessor.makeOutgoingRequest(req);
      }
    })();
  }
      ...
      this.callbacks = {
        ...
        confirm: function () {
          var _confirm = _asyncToGenerator(function* () {
            var requests = yield _this13.inner.confirm();
            for (var m of requests) {
              yield _this13.outgoingRequestProcessor.makeOutgoingRequest(m);
            }
          });
          function confirm() {
