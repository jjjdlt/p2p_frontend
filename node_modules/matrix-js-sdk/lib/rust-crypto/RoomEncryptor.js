...
import { EncryptionAlgorithm, EncryptionSettings, RoomId, UserId, HistoryVisibility as RustHistoryVisibility } from "@matrix-org/matrix-sdk-crypto-wasm";
import * as RustSdkCryptoJs from "@matrix-org/matrix-sdk-crypto-wasm";
import { EventType } from "../@types/event";
import { logger, LogSpan } from "../logger";
...
export class RoomEncryptor {
  ...
  onRoomMembership(member) {
    ...
    if (member.membership == KnownMembership.Join || member.membership == KnownMembership.Invite && this.room.shouldEncryptForInvitedMembers()) {
      ...
      logDuration(this.prefixedLogger, "updateTrackedUsers", /*#__PURE__*/_asyncToGenerator(function* () {
        _this.olmMachine.updateTrackedUsers([new UserId(member.userId)]).catch(e => {
          _this.prefixedLogger.error("Unable to update tracked users", e);
        });
      }));
    }
  ...
  prepareForEncryption(globalBlacklistUnverifiedDevices) {
    ...
    return _asyncToGenerator(function* () {
      ...
      yield _this2.encryptEvent(null, globalBlacklistUnverifiedDevices);
    })();
  }
  ...
  encryptEvent(event, globalBlacklistUnverifiedDevices) {
    ...
    var prom = this.currentEncryptionPromise.catch(() => {
      ...
      yield logDuration(logger, "ensureEncryptionSession", /*#__PURE__*/_asyncToGenerator(function* () {
        yield _this3.ensureEncryptionSession(logger, globalBlacklistUnverifiedDevices);
      }));
      if (event) {
        yield logDuration(logger, "encryptEventInner", /*#__PURE__*/_asyncToGenerator(function* () {
          yield _this3.encryptEventInner(logger, event);
        }));
      }
    }));
  ...
  ensureEncryptionSession(logger, globalBlacklistUnverifiedDevices) {
    var _this4 = this;
    return _asyncToGenerator(function* () {
      var _this4$room$getBlackl;
      if (_this4.encryptionSettings.algorithm !== "m.megolm.v1.aes-sha2") {
        throw new Error("Cannot encrypt in ".concat(_this4.room.roomId, " for unsupported algorithm '").concat(_this4.encryptionSettings.algorithm, "'"));
      }
      logger.debug("Starting encryption");
      var members = yield _this4.room.getEncryptionTargetMembers();
      ...
      if (!_this4.lazyLoadedMembersResolved) {
        yield logDuration(_this4.prefixedLogger, "loadMembersIfNeeded: updateTrackedUsers", /*#__PURE__*/_asyncToGenerator(function* () {
          yield _this4.olmMachine.updateTrackedUsers(members.map(u => new RustSdkCryptoJs.UserId(u.userId)));
        }));
        logger.debug("Updated tracked users");
        _this4.lazyLoadedMembersResolved = true;
        ...
        yield logDuration(_this4.prefixedLogger, "doProcessOutgoingRequests", /*#__PURE__*/_asyncToGenerator(function* () {
          yield _this4.outgoingRequestManager.doProcessOutgoingRequests();
        }));
      } else {
        ...
        _this4.outgoingRequestManager.doProcessOutgoingRequests();
      }
      logger.debug("Encrypting for users (shouldEncryptForInvitedMembers: ".concat(_this4.room.shouldEncryptForInvitedMembers(), "):"), members.map(u => "".concat(u.userId, " (").concat(u.membership, ")")));
      var userList = members.map(u => new UserId(u.userId));
      yield logDuration(_this4.prefixedLogger, "ensureSessionsForUsers", /*#__PURE__*/_asyncToGenerator(function* () {
        yield _this4.keyClaimManager.ensureSessionsForUsers(logger, userList);
      }));
      var rustEncryptionSettings = new EncryptionSettings();
      rustEncryptionSettings.historyVisibility = toRustHistoryVisibility(_this4.room.getHistoryVisibility());
      ...
      if (typeof _this4.encryptionSettings.rotation_period_ms === "number") {
        rustEncryptionSettings.rotationPeriod = BigInt(_this4.encryptionSettings.rotation_period_ms * 1000);
      }
      if (typeof _this4.encryptionSettings.rotation_period_msgs === "number") {
        rustEncryptionSettings.rotationPeriodMessages = BigInt(_this4.encryptionSettings.rotation_period_msgs);
      }
      ...
      rustEncryptionSettings.onlyAllowTrustedDevices = (_this4$room$getBlackl = _this4.room.getBlacklistUnverifiedDevices()) !== null && _this4$room$getBlackl !== void 0 ? _this4$room$getBlackl : globalBlacklistUnverifiedDevices;
      yield logDuration(_this4.prefixedLogger, "shareRoomKey", /*#__PURE__*/_asyncToGenerator(function* () {
        var shareMessages = yield _this4.olmMachine.shareRoomKey(new RoomId(_this4.room.roomId),
        ...
        if (shareMessages) {
          for (var m of shareMessages) {
            yield _this4.outgoingRequestManager.outgoingRequestProcessor.makeOutgoingRequest(m);
          }
        }
      }));
    })();
  ...
  forceDiscardSession() {
    var _this5 = this;
    return _asyncToGenerator(function* () {
      var r = yield _this5.olmMachine.invalidateGroupSession(new RoomId(_this5.room.roomId));
      if (r) {
        _this5.prefixedLogger.info("Discarded existing group session");
      }
    })();
  }
  encryptEventInner(logger, event) {
    ...
    return _asyncToGenerator(function* () {
      ...
      var encryptedContent = yield _this6.olmMachine.encryptRoomEvent(new RoomId(_this6.room.roomId), event.getType(), JSON.stringify(event.getContent()));
      event.makeEncrypted(EventType.RoomMessageEncrypted, JSON.parse(encryptedContent), _this6.olmMachine.identityKeys.curve25519.toBase64(), _this6.olmMachine.identityKeys.ed25519.toBase64());
