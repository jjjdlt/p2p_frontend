...
export function parseErrorResponse(response, body) {
  ...
  if (((_contentType = contentType) === null || _contentType === void 0 ? void 0 : _contentType.type) === "application/json" && body) {
    return new MatrixError(JSON.parse(body), response.status, isXhr(response) ? response.responseURL : response.url);
  }
  if (((_contentType2 = contentType) === null || _contentType2 === void 0 ? void 0 : _contentType2.type) === "text/plain") {
    return new HTTPError("Server returned ".concat(response.status, " error: ").concat(body), response.status);
  }
  return new HTTPError("Server returned ".concat(response.status, " error"), response.status);
}
