...
var add = function(a, b) {
  ...
  return callback && config.onComplete ? function() {
    var _config;
    config.onComplete && (_config = config).onComplete.apply(_config, arguments), callback && callback.apply(void 0, arguments);
  } : callback || config.onComplete;
}, maybeVectorAnim = function(value, config, anim) {
  if (_instanceof(value, import_AnimatedValueXY.default)) {
    var configX = _object_spread({}, config), configY = _object_spread({}, config);
  ...
  } else if (_instanceof(value, import_AnimatedColor.default)) {
    var configR = _object_spread({}, config), configG = _object_spread({}, config), configB = _object_spread({}, config), configA = _object_spread({}, config);
  ...
  var _start = function(animatedValue, configuration, callback) {
    ...
    singleValue.stopTracking(), _instanceof(configuration.toValue, import_AnimatedNode.default) ? singleValue.track(new import_AnimatedTracking.default(singleValue, configuration.toValue, import_SpringAnimation.default, singleConfig, callback)) : singleValue.animate(new import_SpringAnimation.default(singleConfig), callback);
  };
  return maybeVectorAnim(value, config, spring2) || {
    ...
    _startNativeLoop: function(iterations) {
      var singleConfig = _object_spread_props(_object_spread({}, config), {
        iterations
      });
  ...
  var _start2 = function(animatedValue, configuration, callback) {
    callback = _combineCallbacks(callback, configuration);
    var singleValue = animatedValue, singleConfig = configuration;
    singleValue.stopTracking(), _instanceof(configuration.toValue, import_AnimatedNode.default) ? singleValue.track(new import_AnimatedTracking.default(singleValue, configuration.toValue, import_TimingAnimation.default, singleConfig, callback)) : singleValue.animate(new import_TimingAnimation.default(singleConfig), callback);
  };
...
function forkEvent(event3, listener) {
  return event3 ? _instanceof(event3, import_AnimatedEvent.AnimatedEvent) ? (event3.__addListener(listener), event3) : function() {
    typeof event3 == "function" && event3.apply(void 0, arguments), listener.apply(void 0, arguments);
  } : listener;
}
function unforkEvent(event3, listener) {
  event3 && _instanceof(event3, import_AnimatedEvent.AnimatedEvent) && event3.__removeListener(listener);
}
